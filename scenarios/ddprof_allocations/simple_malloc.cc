...
constexpr int REEVALUATE_INTERVAL = 100;

void do_lot_of_allocations(const Options &options, Stats &stats) {
  uint64_t nb_alloc{0};
  ...
  auto prev_alloc_count = nb_alloc;
  auto target_duration = std::chrono::nanoseconds(1000000000 / options.alloc_per_sec);
  auto duration_between_allocs = target_duration;
  ...
  for (uint64_t i = 0; i < options.loop_count; ++i) {
    ...
    // Re-evaluate duration_between_allocs every REEVALUATE_INTERVAL iterations
    if ((i+1) % REEVALUATE_INTERVAL == 0) {
      auto curr_time = std::chrono::steady_clock::now();
      auto curr_alloc_count = nb_alloc;

      // Compute allocation rate over the previous X iterations
      auto current_duration = std::chrono::nanoseconds(std::chrono::duration_cast<std::chrono::nanoseconds>(
                              curr_time - prev_time).count() / (curr_alloc_count - prev_alloc_count));

      duration_between_allocs -= (current_duration - target_duration) / 3;

      // Reset prev_time and prev_alloc_count
      prev_time = curr_time;
      prev_alloc_count = curr_alloc_count;
    }

    // Wait for duration_between_allocs before next allocation
    std::this_thread::sleep_for(duration_between_allocs);
...
void print_stats(pid_t pid, const Stats &stats) {
  ...
  if (fp == NULL) {
    ...
    fprintf(fp, "TestHeaders:%-8s,%-8s,%-14s,%-14s,%-14s,%-14s\n", "PID", "TID",
         "alloc_samples", "alloc_bytes", "wall_time", "cpu_time");
    fprintf(fp, "TestStats  :%-8d,%-8d,%-14lu,%-14lu,%-14lu,%-14lu\n", pid, stats.tid,
         stats.nb_allocations, stats.allocated_bytes, stats.wall_time.count(),
         stats.cpu_time.count());
    fclose(fp);
  }
...
int main(int argc, char *argv[]) {
  try {
    ...
    app.add_option("--malloc", opts.malloc_size,
        ...
        ->default_val(1000);
    app.add_option("--alloc-per-sec", opts.alloc_per_sec,
                   "size of allocations per second")
        ->default_val(400);
