...
async def async_main() -> None:
    ...
    result = 1
    while time.time() < start_time + execution_time_sec:
        result *= math.factorial(10)

    # await asyncio.gather(short_task, my_coroutine(execution_time_sec))

def compute(start_time: float, execution_time_sec: float) -> float:
    result = 1
    print("waiting until", start_time + execution_time_sec, "from", time.time())
    while time.time() < start_time + execution_time_sec:
        result *= math.factorial(10)
        time.sleep(0.001)
...
def sync_main() -> None:
    ...
    start_time = time.time()
    print("computing...")
    compute(start_time, execution_time_sec)
    print("ok")
    # await asyncio.gather(short_task, my_coroutine(execution_time_sec))
