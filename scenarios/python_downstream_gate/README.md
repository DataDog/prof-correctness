...
| mem-domain | `python_mem_domain_3.14` | `python_mem_domain_3.15` | heap-space (wheel-only) |

Extended families (live_heap, cpu, alloc, …) land in follow-up PRs.
...
- Wheel-only `*_3.14` (mem_domain, …) — also in `test_scenarios_exclude` until PyPI ships the feature
