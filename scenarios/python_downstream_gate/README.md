...
Twenty-four scenarios (**3.14 baseline** / **3.15 candidate**) are the default set when dd-trace-py triggers downstream CI.
...
| uvloop | `python_uvloop_3.14` | `python_uvloop_3.15` | wall-time |
| gevent | `python_gevent_3.14` | `python_gevent_3.15` | wall-time + task name |
...
```
python_(cpu|alloc|asyncio|native_cpu|deep_stack|gil_contention|uvloop|gevent|exceptions|async_gen|lock|live_heap)_3\.(14|15)
```
...
TEST_SCENARIOS='python_(cpu|alloc|asyncio|native_cpu|deep_stack|gil_contention|uvloop|gevent|exceptions|async_gen|lock|live_heap)_3\.(14|15)' go test -v -run TestScenarios
```
